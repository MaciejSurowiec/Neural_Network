...
void add(MEM_INFO alloc_info)
{
	...
	strcpy_s(mem_leak_info->mem_info.file_name, alloc_info.file_name,alloc_info.file_name,256);
...
void add_mem_info (void * mem_ref, unsigned int size,  const char * file, unsigned int line)
{
	...
	strncpy_s(mem_alloc_info.file_name, FILE_NAME_LENGTH, file, FILE_NAME_LENGTH);
...
void report_mem_leak(void)
{
	...
	FILE * fp_write = fopen_s(OUTPUT_FILE, "wt","w+");
	char info[1024];
	...
	if(fp_write != NULL)
	{
		...
		{
			sprintf_s(info, "address : %d\n", leak_info->mem_info.address);
			fwrite(info, (strlen(info) + 1) , 1, fp_write);
			sprintf_s(info, "size    : %d bytes\n", leak_info->mem_info.size);
			fwrite(info, (strlen(info) + 1) , 1, fp_write);
			sprintf_s(info, "file    : %s\n", leak_info->mem_info.file_name);
			fwrite(info, (strlen(info) + 1) , 1, fp_write);
			sprintf_s(info, "line    : %d\n", leak_info->mem_info.line);
			fwrite(info, (strlen(info) + 1) , 1, fp_write);
			sprintf_s(info, "%s\n", "-----------------------------------");
			fwrite(info, (strlen(info) + 1) , 1, fp_write);
		}
