...
Network::Network(bool enableExtendedOutput=false) : 
...
int Network::Calculate(PictureMatrix image)
{
	for (int i = 0; i < PictureMatrix::size; i++)
	{
		for (int j = 0; j < PictureMatrix::size; j++)
		{
			layers[inputLayer].GetLayer()[j + (i * PictureMatrix::size)]->
				SetValue(image.GetImage()[j][i]);
		}
	}
